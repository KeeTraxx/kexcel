...
function Sheet(basedir, file, contents, sharedStrings) {
    ...
    function getValue(cell) {
        if (cell) {
            if (cell.$.t == 's') {
                if (sharedStrings[parseInt(cell.v)].hasOwnProperty('t')) {
                    return sharedStrings[parseInt(cell.v)].t[0];
                } else {
                    return _.compact(sharedStrings[parseInt(cell.v)].r.map(function (d) {
    ...
    this.getRowValues = function (rownum) {
        ...
        if (row && row.hasOwnProperty('c')) {
            _.each(row.c, function (cell) {
                result[excelColumnToInt(cell.$.r) - 1] = getValue(cell);
            });
            return result;
        } else {
            return undefined;
