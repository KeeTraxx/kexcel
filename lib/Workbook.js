...
function Workbook(file,callback) {
    ...
    var parseSheets = function () {
        ...
        async.parallel([function(done){
            new XMLFile(path.join(workbook.temppath, '/xl/workbook.xml'), done);
        }], function(err, results){
            workbook.workbookXml = results[0];
        });

        workbook.relationshipXml = new XMLFileOld(path.join(workbook.temppath, '/xl/_rels/workbook.xml.rels'));
        workbook.contentTypesXml = new XMLFileOld(path.join(workbook.temppath, '/[Content_Types].xml'));
        ...
        _.each(workbook.relationshipXml.findall('.//Relationship[@Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet"]'), function (sheetdata) {
            console.log(workbook.workbookXml.xml);
            ...
            var attribs = {
                ...
                sheetId: parseInt(workbooksheet.attrib.sheetId)
            };
            workbook.sheets.push(new Sheet(workbook, attribs));
        });

        workbook.sheets.sort(function(a,b){
