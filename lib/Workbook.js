...
function Workbook(file) {
    ...
    var parseSheets = function() {
        ...
        if ( sharedstringsfile ) {
            ...
            r.attrib.Id = 'rkId'+workbook.relationshipXml.findall('.//Relationship').length+1;
            workbook.sharedStringsXml = new SharedStrings(path.join(workbook.temppath, 'xl', filename), '<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="3" uniqueCount="3"><si><t>test</t></si><si><t>test2</t></si><si><t>test3</t></si></sst>');
        }
    ...
    this.pipe = function(output, callback) {
        ...
        this.sharedStringsXml.xml.save();
        ...
        archive.pipe(output);
        archive.bulk([{expand: true, cwd: workbook.temppath, src: ['**','_rels/.rels'], data:{ date: new Date() } }]);
        archive.finalize();
    }
