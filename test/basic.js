...
describe('KExcel sheet modification', function () {
    ...
    it('should return undefined for empty rows', function () {
        var sheet = workbook.getSheet('FooSheet');
        expect(sheet.getRow(42)).to.be.undefined;
    });

    it('should be able to add rows with some undefined or null values', function () {
        var sheet = workbook.getSheet('FooSheet');
        var rownum = sheet.appendRow(['This', null, 'row', undefined, 'has', undefined, 'values']);
        expect(sheet.getCellValue(rownum,2)).to.be.undefined;
        expect(sheet.getCellValue(rownum,4)).to.be.undefined;
        expect(sheet.getCellValue(rownum,6)).to.be.undefined;
        sheet.getRow(rownum).should.include.members(['This', 'row', 'has', 'values']);
    });

    it('should return last row = 0 for an empty sheet', function () {
        var sheet = workbook.createSheet('EmptySheet');
        sheet.getLastRowNumber().should.equal(0);
    });

    it('should be able to append a row in an empty sheet', function () {
        var sheet = workbook.getSheet('EmptySheet');
        var rownum = sheet.appendRow([1,2,3]);
        sheet.getRow(rownum).should.eql([1,2,3]);
    });

    after(function (done) {
        //var stream = workbook.pipe(fs.createWriteStream('basic2.xlsx'));
        var stream = workbook.pipe(devnull());
        stream.on('finish', done);
    });
